					...
					<ul class="nav nav-inline admin-nav">
						...
						<li class="active"><a href="admin/testQuestion/index" class="icon-shopping-cart"> 试卷</a>
						    <ul>
								...
								<li class="active"><a href="admin/testQuestion/update">修改试卷</a></li>
							</ul>
						</li>
...
<div class="admin">
    <form>
    <input type="hidden" id="updateid" value="${testQuestion.tqid}">
				<div class="panel admin-panel">
					...
					<table>
					<tr>
					<td class="my_td">试题类型:<select id="couid" name="couid" data-validate="required:请选择,length#>=1:至少选择1项">
...
<div class="hidden">
		    ...
		    $(document).ready(function(){
				$('#exam_update_form').bind('submit', function(){
			...
			<script>
			var couid=$("#couid").val();
			$("#course_search").bind("submit",function(){
				
				$.post("admin/testQuestion/questionlist",{"couid":couid},function(data){
					var total = data.pi;
	                var pageS = total.pc.maxpage;
	                var pageIndex = total.pc.currentindex;
	                PageClick(pageIndex, total, pageS,3);
				});
				PageClick = function (pageIndex, total, pageS, spanInterval) {
	                $.ajax({
	                    url: "admin/testQuestion/questionlist",
	                    data: {"currindex":pageIndex,"couid":"${testQuestion.couid==null?1:testQuestion.couid}"},
	                    type: "post",
	                    dataType:"json",
	                    beforeSend: function(){
	                    	changeIds();
	                    ...
	                    success: function (data) {
	                        ...
	                        //向表格中添加内容
	for (var i =0; i < total.lists.length; i++) {
	                            table.append(
	                                $("<tr><td>"+
	                                "<input type='checkbox' name='id' value="+total.lists[i].titid+" />"
	 		                        +"</td><td>"+
	 		                       total.lists[i].titid
	                                +"</td><td>"+
	                                total.lists[i].titname
	                                +"</td><td>"+
	                                total.lists[i].couname
	                                +"</td><td>"+
	                                total.lists[i].titscore
	                                +"</td></tr>")
	                                );
	                        } //for
	...
	getChecked();
			...
			var checkedIds="";//翻页保存选中的id
			 ...
			 $(function () {
		            ...
		            $.post("admin/testQuestion/questionlist", {"couid":"${testQuestion.couid==null?1:testQuestion.couid}"}, function (data) {
		                var total = data.pi;
		                var pageS = total.pc.maxpage;
		                var pageIndex = total.pc.currentindex;
		                PageClick(pageIndex, total, pageS,3);
		...
		getChecked();
		       ...
		       function doUpdate(){
		    	   changeIds();
		    	   var id = ${testQuestion.tqid==null?1:testQuestion.tqid};
		    	   var content = checkedIds;
		    	   $.ajax({
	                    url: "admin/testQuestion/doUpdate",
	                    data: { "tqid": id, "content":content},
	                    type: "post",
	                    dataType: "json",
	                    success: function (data){
	                    	alert(data.message);
