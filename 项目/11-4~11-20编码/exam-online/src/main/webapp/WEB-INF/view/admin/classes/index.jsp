	...
	<body>
					...
					<ul class="nav nav-inline admin-nav">
						...
						<li class="active"><a href="admin/user/index" class="icon-user"> 用户</a>
						    <ul>
								...
								<li><a href="#">教师管理</a></li>
								<li><a href="#">学生管理</a></li>
								<li class="active"><a href="admin/classes/index">班级管理</a>
		...
		<div class="admin">
			<form>
				<div class="panel admin-panel">
					...
					<form method="post" action="admin/classes/index">
					<table>
					<tr>
					...
					<div class="padding border-bottom">
						...
						<input type="button" id="create_group" class="button button-small border-green" value="创建班级" />
					...
					<table class="table table-hover">
						...
						<c:forEach items="${pi.lists}" var="classes">
						<tr>
							<td>
								<input type="checkbox" name="id" value="${classes.claid}" />
							</td>
							<td>${classes.claid}</td>
							<td>${classes.claname}</td>
							<td>${classes.deptname}</td>
							<td><a class="button border-blue button-little" href="admin/classes/update?id=${classes.claid}">修改</a> <a class="button border-yellow button-little" style="cursor:pointer;" onclick="doDelete(${classes.claid})">删除</a></td>
						</tr>
						</c:forEach>
					</table>
				    ...
				    <li style="border: 0;"><a>共计：${pi.pc.pagecount}页/${pi.pc.rscount}条记录</a>
						...
						<li><c:if test="${pi.pc.currentindex==1}" var="fp">
								...
								<a
									href="admin/classes/index?currindex=${pi.pc.currentindex-1}
										<c:if test='${classesname!=null}'>
										   &claname=${classesname.claname}
										   </c:if>
										">上一页</a>
							</c:if></li>
						...
						<c:forEach begin="${pi.pc.minpage}" step="1"
							...
							<li><c:if test="${pi.pc.currentindex==index}" var="t">
									...
									<a
										href="admin/classes/index?currindex=${index}
										   <c:if test='${classesname!=null}'>
										   &claname=${classesname.claname}
										   </c:if>
										">${index}</a>
						...
						<li><c:if test="${pi.pc.currentindex==pi.pc.pagecount}"
								...
								<a
									href="admin/classes/index?currindex=${pi.pc.currentindex+1}
										<c:if test='${classesname!=null}'>
										   &classesname=${classesname.claname}
										   </c:if>
										">下一页</a>
							</c:if></li>
					</ul>
		...
		<div class="hidden" id="group_form" style="padding: 20px;">
			...
			<form id="group_add_form" method="post"
				action="admin/user/groupupdate">
				<div class="form-group">
					<div class="label">
						<label>班级名称</label>
					</div>
					<div class="field">
						<input type="text" class="input" name="claname" maxlength="50"
