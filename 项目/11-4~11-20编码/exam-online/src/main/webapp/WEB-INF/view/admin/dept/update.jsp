				...
				<div class="admin-navbar">
					...
					<ul class="nav nav-inline admin-nav">
						...
						<li class="active"><a href="admin/user/index" class="icon-user"> 用户</a>
						    <ul>
								<li><a href="admin/user/idnex">用户管理</a></li>
								<li><a href="#">教师管理</a></li>
								<li><a href="#">学生管理</a></li>
...
<div class="admin">
    <form method="post" id="dept_update_form" action="admin/dept/doUpdate">
    <input type="hidden" id="deptid" value="${sessionScope.dept.deptid}">
...
<div class="hidden">
		    ...
		    $(document).ready(function(){
				$('#dept_update_form').bind('submit', function(){
			...
			var checkedIds="";//翻页保存选中的id
			 ...
			 $(function () {
		            ...
		            $.post("admin/classes/classeslist", null, function (data) {
		            ...
		            PageClick = function (pageIndex, total, pageS, spanInterval) {
		                $.ajax({
		                    url: "admin/classes/classeslist",
		                    data: { "currentindex": pageIndex },
		                    type: "post",
		                    dataType: "json",
		                    beforeSend: function(){
		                    	changeIds();
		                    ...
		                    success: function (data) {
		                        ...
		                        //table.append("<th>选择</th><th>id</th><th>试卷名称</th><th>试卷类型</th><th>分值</th>");
                                 if(total.lists.length<=0){
                                	 table.text("没有新增");
                                	 table.innerText="没有新增的班级,点击";
                                	 table.innerHtml="<a href='admin/classes/index'>这里</a>";
                                	 table.innerText="新增";
                                 }
		                        //向表格中添加内容
		for (var i =0; i < total.lists.length; i++) {
		                            table.append(
		                                $("<tr><td>"+
		                                "<input type='checkbox' name='id' value="+total.lists[i].claid+" />"
		 		                        +"</td><td>"+
		 		                       total.lists[i].claid
		                                +"</td><td>"+
		                                total.lists[i].claname
		                                +"</td></tr>")
		                                );
		                        } //for
		...
		getChecked();
		       ...
		       function doUpdate(){
		    	   ...
		    	   var claids = checkedIds;
		    	   $.ajax({
	                    url: "admin/dept/doUpdate",
	                    data: { "deptid": deptid, "claids": claids },
	                    type: "post",
	                    dataType: "json",
	                    success: function (data) {
	                    	alert("添加成功！");
