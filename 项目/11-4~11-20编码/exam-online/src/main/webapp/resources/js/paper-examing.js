...
// 		
	...
	finishExam : function finishExam() {
		...
		efdata.examid=$("#exam-id").val();
		efdata.tqid=$("#paper-id").val();
		efdata.stuid=$("#stu-id").val();
		
		//data.startTime=$("#start-time").val();
		
		$("#question-submit button").attr("disabled", "disabled");
		var date=JSON.stringify(efdata)
		var request = $.ajax({
			headers : {
